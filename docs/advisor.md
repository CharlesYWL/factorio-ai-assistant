...
告警的证据和建议按当前游戏语言称呼产品、流体、科技和机器：`铁板` 而不是
`iron-plate`。名称来自 Mod 通过 `localization_update` 同步的官方翻译（见
`docs/protocol.md`），规则判定、告警 ID 与去重仍然只用 prototype ID。

尚未同步到名称时的回退顺序是：游戏翻译 → 规则自带的当前语言措辞（仅限规则固定
引用的少数 ID，例如润滑油、重油、石油气）→ prototype ID。因此离线或刚连接时正文
仍然可读，只是可能显示英文 ID。
