...
| `find_machines` | 配方 / 状态 / 原型名 / 类型 / 有无模块，至少给一个 | 全图匹配的机器，按产线聚合成最多 24 组 |
| `highlight_entities` | 最多 12 个标注；每个要么给选区内真实存在的 `unit`，要么给 `x`/`y` 坐标 | 在游戏世界与地图上画出彩色方框、文字标签和地图标签 |

### 全图搜索

`find_machines` 是唯一一个**会离开 Companion 进程**的工具。其余工具查的都是已缓存的
数据，而「重炮炮弹产线在哪」只有游戏知道——Companion 从来只被动接收 Mod 推来的东西。

所以它走一次 UDP 往返：Companion 发 `search_request`，Mod 扫图后回 `search_response`，
按 `reply_to` 配对（中间可能夹杂其他包，到达顺序说明不了什么）。10 秒无响应就放弃，
一个静默挂起的搜索会耗光工具循环的时限，让玩家白等。

Mod 侧的代价控制：

- **只在提问时扫**，不像矿脉那样定时跑。绝大多数问题用不到位置信息，为它持续付费不划算。
- 按**原型类型**交给引擎筛选，而不是自己遍历 chunk。
- 相邻机器**聚合成产线**：忘了产线在哪，需要的是「那片地方」，不是四十个坐标。
- 匹配上限 2000 台、开放分组上限 128 个（聚类是 O(n×组数)，不封顶会卡住游戏线程）、
  返回上限 24 组。任何一处触顶都会把 `truncated` 标出来，而不是悄悄少报。

**必须至少给一个筛选条件**——空筛选等于扫描整个工厂，既没用又贵。
...
> **说过就得做到**：模型可能写下「已在游戏内标记」却根本没调用工具。Companion 以
> **实际发生的工具调用**为准——回答里出现这类说法但一个标注都没产生时，会追加一句
> 更正并记 `assistant_marker_claim_unmet` 日志。玩家去地图上找不到东西，比没有标注
> 功能更糟。
